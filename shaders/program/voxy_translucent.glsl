...
const float PHOTON_WATER_BASE_SURFACE_ALPHA = 0.01;
...
void getPhotonVoxyThinWaterSurface(
    ...
    surfaceAlpha = PHOTON_WATER_BASE_SURFACE_ALPHA;
    ...
    surfaceAlbedo = clamp(
        0.5 * exp(-2.0 * PHOTON_WATER_ABSORPTION) * textureHighlight,
        0.0,
        1.0
    );
    ...
    float normalLayerDistance = getVoxyWaterLayerDistance(surfaceViewPosition)
...
void emitSdvWater(VoxyFragmentParameters parameters) {
    ...
    // Photon keeps Voxy's centred 16x16 lightmap coordinates for water all the
    // way into distant_water.glsl.  SDV's generic Voxy decode deliberately
    // snaps the first light levels to zero; doing that here removes the sky
    // illumination which supplies the blue in-scattering on dim LOD water.
    // Keep the generic decode for non-water materials, but preserve Photon's
    // exact light-level domain for both the thin surface and volume resolver.
    lmCoord = clamp(parameters.lightMap, vec2(0.0), vec2(1.0));
    #ifdef WORLD_CUSTOM_SKYLIGHT
