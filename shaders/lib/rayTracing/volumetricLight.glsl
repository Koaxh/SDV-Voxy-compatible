...
vec3 getVolumetricLight(in vec3 nFeetPlayerPos, in float feetPlayerDist, in float fogFactor, in float borderFog, in float dither, in bool isSky){
	...
	// Border fog
		...
		for(uint i = 0u; i < VOLUMETRIC_LIGHT_STEPS; i++){
			...
			vec3 volumeShadowPosition = vec3(
				startPos.xy / (length(startPos.xy) * 2.0 + 0.2),
				startPos.z * 0.1
			) + 0.5;
			vec3 volumeShadow = getShdCol(volumeShadowPosition);
			if(isEyeInWater == 1){
				volumeShadow = getUnderwaterShdCol(volumeShadow, fogColor);
			}
			volumeData += volumeShadow;
			// We continue tracing!
			startPos += endPos;
		}
		
		vec3 effectiveLightCol = lightCol;
		if(isEyeInWater == 1){
			// Preserve full godray luminance while shifting the hue to a natural cyan-green / teal aquatic tone
			float maxLum = max(lightCol.r, max(lightCol.g, lightCol.b));
			vec3 waterTint = normalize(toLinear(clamp(fogColor, vec3(0.0), vec3(1.0))) * vec3(0.8, 1.25, 0.95) + vec3(0.02, 0.32, 0.26));
			effectiveLightCol = mix(lightCol, waterTint * maxLum * 1.6, 0.75);
		}

		return volumeData * effectiveLightCol * (min(1.0, VOLUMETRIC_LIGHTING_STRENGTH + VOLUMETRIC_LIGHTING_STRENGTH * isEyeInWater) * squared(heightFade) * volumetricFogDensity * volumetricStepsInverse);
	#else
