...
vec3 rayTraceScene(in vec3 screenPos, in vec3 viewPos, in vec3 rayDir, in float dither){
    ...
    bool getVoxyRayTraceSceneDepth(in vec2 screenCoord, out float sceneDepth){
        ...
        if(vanillaDepth < 1.0){
            ...
            vec3 vanillaViewPos = getViewPos(
                gbufferProjectionInverse,
                vec3(screenCoord, vanillaDepth)
            );
            // Photon treats Voxy as a projection-only depth domain. The
            // reconstructed vanilla position is already in the same view
            // basis expected by vxProj.
            vec4 voxyClipPos = vxProj * vec4(vanillaViewPos, 1.0);
            if(abs(voxyClipPos.w) < 0.000001) return false;

            float voxyNdcDepth = voxyClipPos.z / voxyClipPos.w;
