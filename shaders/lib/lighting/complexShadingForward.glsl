#if defined VOXY && defined SHADOW_MAPPING && (defined TERRAIN || defined VOXY_FORWARD_SHADOW_RECEIVER) && !defined DH_GBUFFERS
	#ifndef VOXY_PATCH
	...
	float getVanillaTemporalVoxyCastShadow() {
		vec3 previousFeetPlayerPos = vertexFeetPlayerPos
...
vec3 complexShadingForward(in dataPBR material){
	...
	totalIllumination *= material.ambient;
		...
		#ifdef VOXY_PATCH
			// shaders.properties defines this as the exact Z column used below.
			// Use Voxy's supported vec3 uniform path instead of re-reading the
			// equivalent direction through its separate mat4 uniform adapter.
			float NLZ = dot(material.normal, voxyLightDir);
		#else
		...
		bool isSubSurface = material.ss > 0;
			...
			vec3 shdCol = vec3(0);
			float translucentCasterCoverage = 0.0;
			...
			if(isShadow || isSubSurface){
				...
				// Sample shadows
					...
					if(isEyeInWater == 1){
						shdCol = getUnderwaterShdCol(shdCol, fogColor);
					}
				#endif
				...
				#if defined TERRAIN || defined WATER
					if(isEyeInWater == 0){
						shdFactor *= min(1.0, (lmCoord.y + eyeBrightFact) * 4.0);
					} else if(isEyeInWater == 1){
						// Valid native water shadows retain full SDV illumination and
						// caustics. Missing Voxy/out-of-range water casters use the
						// skylight fallback that prevents border leaks.
						shdFactor *= mix(
							skyLightSquared,
							1.0,
							translucentCasterCoverage
		...
		#if defined VOXY && defined SHADOW_MAPPING && (defined TERRAIN || defined VOXY_FORWARD_SHADOW_RECEIVER) && !defined DH_GBUFFERS
			// Preserve the vanilla CSM/Lambert result and add only Voxy casters.
			shdCol *= getVanillaTemporalVoxyCastShadow();
		#endif
