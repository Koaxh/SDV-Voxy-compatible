vec3 complexShadingDeferred(in vec3 sceneCol, in vec3 screenPos, in vec3 viewPos, in vec3 normal, in vec3 albedo, in vec3 dither, in float viewDotInvSqrt, in float metallic, in float smoothness, in bool realSky
	...
	// Calculate SSGI
		...
		vec3 SSGIrayDir = generateCosineVector(normal, noiseUnitVector);
		#ifdef SDV_VOXY_DEFERRED_RAYTRACE
			vec3 SSGIcoord = voxyRayTrace
				? rayTraceSceneVoxy(screenPos, viewPos, SSGIrayDir, dither.z)
				: rayTraceScene(screenPos, viewPos, SSGIrayDir, dither.z);
		#else
			vec3 SSGIcoord = rayTraceScene(screenPos, viewPos, SSGIrayDir, dither.z);
		#endif
		...
		// If sky don't do SSGI
				...
				if(voxyRayTrace){
					vec2 prevSSGICoord;
					if(SSGIcoord.z > 0.5 && getPrevVoxyRayTraceScreenCoord(SSGIcoord.xy, prevSSGICoord)){
						sceneCol += albedo * textureLod(colortex5, prevSSGICoord, 0).rgb;
					}
				} else {
	...
	// vxProj owns a separate depth projection, not a separate view basis.
	// Both vanilla and Voxy positions entering this function are already in the
	// current gbuffer view domain.
	vec3 skyReflectViewDir = reflectViewDir;

	// Calculate SSR and sky reflections
	#ifdef SSR
		// Get SSR screen coordinates
		#ifdef SDV_VOXY_DEFERRED_RAYTRACE
			vec3 SSRCoord = voxyRayTrace
				? rayTraceSceneVoxy(screenPos, viewPos, reflectViewDir, dither.z)
				: rayTraceScene(screenPos, viewPos, reflectViewDir, dither.z);
		#else
		...
		if(SSRCoord.z < 0.5){
			...
			if(reflectDirF.z < viewPos.z){
				#ifdef SDV_VOXY_DEFERRED_RAYTRACE
					if(voxyRayTrace){
						vec3 SSRDH = getVoxyRayTraceScreenPos(reflectDirF);
						float fallbackDepth;
						if(SSRDH.x >= 0.0 && SSRDH.y >= 0.0
						&& SSRDH.x <= 1.0 && SSRDH.y <= 1.0
						&& getVoxyRayTraceSceneDepth(SSRDH.xy, fallbackDepth)){
							SSRCoord = vec3(SSRDH.xy, 1.0);
						}
					} else {
			...
			// Get reflections and check for sky
				...
				if(voxyRayTrace){
					vec2 prevSSRCoord;
					bool validHistoryHit = SSRCoord.z > 0.5
						&& getPrevVoxyRayTraceScreenCoord(SSRCoord.xy, prevSSRCoord);
					reflectCol = validHistoryHit
						? textureLod(colortex5, prevSSRCoord, 0).rgb
						: getSkyReflection(skyReflectViewDir);
				} else {
					reflectCol = SSRCoord.z < 0.5 ? getSkyReflection(skyReflectViewDir) : textureLod(colortex5, getPrevScreenCoord(SSRCoord.xy), 0).rgb;
				}
			#else
				vec3 reflectCol = SSRCoord.z < 0.5 ? getSkyReflection(skyReflectViewDir) : textureLod(colortex5, getPrevScreenCoord(SSRCoord.xy), 0).rgb;
			#endif
		#else
			// Get reflections and check for sky
			vec3 reflectCol = SSRCoord.z < 0.5 ? getSkyReflection(skyReflectViewDir) : textureLod(colortex4, SSRCoord.xy, 0).rgb;
		#endif
	#else
		vec3 reflectCol = getSkyReflection(skyReflectViewDir);
	#endif
