...
vec3 getSkyFogRender(in vec3 nEyePlayerPos){
    // If player is in water, return underwater ambient fog color
    if(isEyeInWater == 1){
        vec3 waterFogLinear = toLinear(clamp(fogColor, vec3(0.0), vec3(1.0))) * vec3(0.8, 1.25, 0.95);
        vec3 waterAmbient = skyCol * vec3(0.7, 1.15, 0.9) + toLinear(AMBIENT_LIGHTING + nightVision * 0.5) + lightCol * vec3(0.15, 0.32, 0.25) * eyeBrightFact;
        return waterFogLinear * waterAmbient * 2.5;
    }
    ...
    if(isEyeInWater == 2) return fogColor;
            ...
            // If current world uses shader sun and moon but not vanilla sun and moon
                ...
                float sunMoonShape = getSunMoonShape(skyPos.xy) * sunMoonIntensitySqrd;
					...
					// Underwater forward Mie scattering sun flare & radiant halo
                if(isEyeInWater == 1 && skyPos.z > 0.0){
                    float sunAngle = max(0.0, skyPos.z);
                    float a2 = sunAngle * sunAngle;
                    float a4 = a2 * a2;
                    float a8 = a4 * a4;
                    float a16 = a8 * a8;
                    float a32 = a16 * a16;
                    float a64 = a32 * a32;
                    float coreGlow = a64 * 12.0;
                    float wideCorona = a8 * 2.5;
                    currSkyCol += (sRGBSunCol + vec3(1.5)) * (coreGlow + wideCorona) * (1.0 - rainStrength);
                }
                #endif
            #endif
