...
// One representable step in the R8 Voxy shadow-history attachment.
// Zero means "not calculated"; values at or above this marker are valid.
const float OSIEBCA = 1.0 / 255.0;

// Reserve zero as the invalid-history sentinel without biasing valid shadows.
float encodeVoxyShadowHistory(in float shadow) {
	return mix(OSIEBCA, 1.0, clamp(shadow, 0.0, 1.0));
}

float decodeVoxyShadowHistory(in float encodedShadow) {
	if (encodedShadow < OSIEBCA * 0.5) return 1.0;
	return clamp((encodedShadow - OSIEBCA) / (1.0 - OSIEBCA), 0.0, 1.0);
}
...
// SRGB to linear
float toLinear(in float x){ return ((0.315206 * x + 0.6709592) * x + 0.0153882) * x; }
vec3 toLinear(in vec3 x){ return ((0.315206 * x + 0.6709592) * x + 0.0153882) * x; }

// Linear to SRGB
float toSRGB(in float x){ return sqrt(max(x, 0.0)) * 1.14374 - x * 0.145133; }
vec3 toSRGB(in vec3 x){ return sqrt(max(x, 0.0)) * 1.14374 - x * 0.145133; }
