...
/// -------------------------------- /// Fragment Shader /// -------------------------------- ///
    ...
    void main(){
        ...
        vec3 matRaw0 = texelFetch(colortex3, screenTexelCoord, 0).xyz;
            ...
            vxDepthZeroToOne = int(voxyLayerZeroToOne);
        ...
        sceneColOut = texelFetch(colortex4, screenTexelCoord, 0).rgb;
            ...
            if(resolvedWater){
                ...
                bool waterZeroToOne = voxyLayerZeroToOne;
                float waterLayerDistance = photonWaterLayerDistance(
                    texCoord,
                    viewPos,
                    voxyWater,
                    waterZeroToOne
                ...
                // Both owners now use the same Photon ordering and exactly the
                // same body/normal/highlight/reflection function: RGB volume
                // first, then the independent premultiplied thin surface.
                sceneColOut = sceneColOut * (1.0 - waterBody.a)
                    + waterBody.rgb;
        ...
        sceneColOut *= 1.0 - darknessLightFactor;
            ...
            vec2 cloudData = volumetricClouds(nFeetPlayerPos, cloudStartPos0, feetPlayerDist, cloudRenderDistance, dither.x, isSky);
                ...
                cloudData = max(volumetricClouds(nFeetPlayerPos, cloudStartPos1, feetPlayerDist, cloudRenderDistance, dither.x, isSky).yx, cloudData);
            #endif
